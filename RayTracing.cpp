...
color ray_color(const ray& r, const color& background, const hittable& world,shared_ptr<hittable>&lights,int depth) {
    ...
    return emitted + srec.attenuation * rec.mat_ptr->scattering_pdf(r,rec,scattered)
        *ray_color(scattered, background, world, lights,depth - 1)/pdf_val;//每次递归颜色都会衰减
    //point3 target = rec.p + rec.normal + random_unit_vector();//随机点s
    //return 0.5 * ray_color(ray(rec.p, target - rec.p), world,depth-1);
    
    //vec3 unit_direction = unit_vector(r.direction());
    //double t = 0.5 * (unit_direction.y() + 1.0);//[-1,1]->[0,1]
    ////线性差值
...
hittable_list random_scene() {
    ...
    auto checker = make_shared<checker_texture>(color(0.3, 0.2, 0.1),color(0.8,0.8,0.8));
        ...
        for (int b = -11; b < 11; b++) {
            ...
            if ((center - point3(4, 0.2, 0)).length() > 0.9) {
                ...
                if (choose_mat < 0.8) {
                    ...
                    sphere_material = make_shared<lambertian>(albedo);//make_shared后接构造函数，材质指针指向该材质
                    //world.add(make_shared<sphere>(center, 0.2, sphere_material));//用该材质构造球
...
hittable_list cornell_box() {
    ...
    shared_ptr<material> aluminum = make_shared<metal>(color(0.8, 0.85, 0.88), 0.1);
